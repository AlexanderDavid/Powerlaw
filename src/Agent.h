...
protected:
   ...
   *  <ul>
   *		<li>A driving force \f$\mathbf{F}_i\f$ indicating the preference
   of the agent \f$i\f$ to walk in a certain direction at a certain speed, as
   defined in [D. Helbing, I. Farkas, and T. Vicsek, Nature 407, 487 (2000)],
   \f$	\mathbf{F}_{i} = \frac{\mathbf{v}^{pref}_{i}-\mathbf{v}_{i}}{\xi}\f$.
   *      This force can be replaced by a self-propelled force to simulate
   agents with no preferred direction of motion following the approach of [D.
   Grossman, I. S. Aranson, and E. B. Jacob, New J. Phys. 10, 023036 (2008).]
   </li>
   *		<li>The agent-agent interaction force \f$\mathbf{F}_{ij}\f$
   derived in Eq. (S2) of the Supplemental material. </li> *		<li>A
   similar force \f$\mathbf{F}_{iO}\f$ acting on agent \f$i\f$ as a result of
   interaction with each static obstacle <i>O</i> present in the environment.
   *		In our simulations, we generally assume that obstacles are
   modeled as a collection of line segments. Then, \f$\mathbf{F}_{iO} =
   -\nabla_{\mathbf{r}}\left( k\tau^{-2}e^{-\tau/\tau_0} \right)\f$,
   *      but now \f$\tau\f$ denotes the minimal intersection time between the
   ray \f$\mathbf{x}_{i} + t\mathbf{v}_{i}, t >0 \f$ and the 2D capsule
   resulting after sweeping \f$O\f$ with the disc of the agent. *
   </li> *	</ul>
   */
