...
SimulationEngine::~SimulationEngine() {
  ...
  for (vector<Agent *>::iterator it = _agents.begin(); it != _agents.end();
       ++it) {
    delete *it;
    *it = 0x0;
  }

  for (vector<LineObstacle *>::iterator it = _obstacles.begin();
...
void SimulationEngine::init(float xRange, float yRange) {
  ...
  _spatialDatabase = new SpatialProximityDatabase(
      Vector2D(), Vector2D(xRange, yRange), Vector2D(10.0f, 10.0f));
}
...
void SimulationEngine::exportSimulation(std::ofstream &file) {
  for (unsigned int i = 0; i < _agents.size(); ++i) {
    if (_agents[i]->enabled())
      file << _agents[i]->id() << "," << _agents[i]->position().x << ","
           << _agents[i]->position().y << "," << _agents[i]->radius() << ","
           << _globalTime << std::endl;
  }
}
...
void SimulationEngine::addAgent(Vector2D position, Vector2D goal,
